...
repo=Path(sys.argv[1]).resolve();prefix=Path('.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-16');root=repo/prefix
...
write(root/'selection-final.json',{'directExpected':expected,'newArchive':str(prefix/'transport-01/catalog.json') if repo.name=='abiogenesis' else None,'excludedRuntimeScope':'see exclusions.json','previousO4Bank':'reference only; no historical bytes reprocessed'})
...
if repo.name=='abiogenesis':
 for p,e in json.loads((root/'source-postimages.json').read_text()).items():assert ident(repo/p)==e,p
