...
A=Path('/Users/jim/src/apps/abiogenesis');G=Path('/Users/jim/src/apps/odd_glc');repo=Path(sys.argv[1]).resolve();root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-13';previous=root.parent/'successor-12'
...
def add(p,expect=None):
 ...
 assert not any(x in rel for x in ['/fresh-native-01/resources/','/fresh-native-01/opus-successor-01/caller/','/fresh-native-01/opus-successor-02/']),rel
...
def freeze(f,expected=None,base_override=None):
 ...
 rows=next((d[k] for k in ['members','files','entries','delta','carrierFiles','ownEvidence'] if isinstance(d.get(k),list)),None)
