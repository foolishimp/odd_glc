...
A=Path('/Users/jim/src/apps/abiogenesis');G=Path('/Users/jim/src/apps/odd_glc');repo=Path(sys.argv[1]).resolve();root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-05';previous=root.parent/'successor-04'
...
def freeze(f,expected=None,base_override=None):
 ...
 rows=d.get('members',d.get('files',d.get('delta')));assert isinstance(rows,list),(str(f),list(d))
 for r in rows+d.get('externalCallerMembers',[]):
  raw=Path(r['path'])
  ...
  add(p,{'sha256':r['sha256'],'byteLength':r.get('bytes',r.get('byteLength',r.get('byteCount')))})
