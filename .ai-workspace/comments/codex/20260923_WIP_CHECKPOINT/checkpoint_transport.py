...
def prepare(repo, out, held, included=None):
    ...
    roots = [repo / q for q in included] if included else [repo / '.ai-workspace/comments']
    if not included and repo.name == 'odd_glc':
    ...
    value = {'kind': 'wip_checkpoint_transport', 'repository': str(repo),
        ...
        'scope': 'WIP preservation only; current generic run failed before assessment; no release, qualification, RC or scenario-completion claim.',
