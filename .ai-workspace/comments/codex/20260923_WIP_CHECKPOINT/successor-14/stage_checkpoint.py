...
repo=Path(sys.argv[1]).resolve();A=Path('/Users/jim/src/apps/abiogenesis');root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-14';rel=root.relative_to(repo).as_posix()
...
for p in set(archived)|set(direct):assert not any(x in p for x in ['/fresh-native-01/resources/','/fresh-native-01/opus-successor-01/caller/','/fresh-native-01/opus-successor-03/']),p
ignore=repo/'.ai-workspace/comments/.gitignore';ignoreBeforeSha=sha(ignore);assert git('show','HEAD:.ai-workspace/comments/.gitignore')==ignore.read_bytes()
def literal(p):return ''.join('\\'+c if c in '\\[]*?!# 'else c for c in p)
with ignore.open('a')as f:f.write('\n# WIP successor-14: exact lossless transport-backed CLOSED originals; active resources excluded.\n'+'\n'.join('/'+literal(p.removeprefix('.ai-workspace/comments/'))for p in sorted(archived))+'\n')
...
write('transport-backed-ignore-receipt.json',{'path':'.ai-workspace/comments/.gitignore','preimageSha256':ignoreBeforeSha,'preimageSource':base+':.ai-workspace/comments/.gitignore','postimageSha256':sha(ignore),'exactIgnoredOriginals':sorted(archived),'activeRootsAreNotIgnored':exclusions})
...
write('composition.json',{'basisCommit':base,'inherited':'successor-13/composition.json and unchanged prior bank indexes','newBanks':banks,'newMembers':len(archived),'newLogicalBytes':sum(x['byteLength']for x in archived.values()),'newTransportBytes':sum(b['transportBytes']for b in banks),'priorPayloadReadOrReupload':False,'scope':scope,'excluded':exclusions,'rootAcceptance':accept})
