...
A=Path('/Users/jim/src/apps/abiogenesis');G=Path('/Users/jim/src/apps/odd_glc');repo=Path(sys.argv[1]).resolve();root=repo/'.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-03';previous=root.parent/'successor-02'
...
def add(p,expect=None):
 ...
 assert '/WHOLE_PATH_SCOPE_REVIEW/'not in rel.upper(),rel
 ...
 if rel in selected:assert selected[rel]['expectedFreeze']==e,('Conflicting frozen identity',rel);return
 ...
 if rel in prior and prior[rel]['sha256']==e['sha256']:
  ...
  # Stable banked originals reuse their accepted proof. Only a selected post-checkpoint rewrite is compared.
  if st.st_mtime_ns>epoch:assert sha(p)==e['sha256'],('Banked selected member moved',rel)
...
def freeze(f,expected=None):
 f=Path(f);d=json.loads(f.read_text());actual=sha(f)
 if expected:assert actual==expected,('Freeze moved',str(f))
 base=Path(d.get('root',d.get('basePath',str(f.parent))))
 rows=d.get('members',d.get('files'));assert isinstance(rows,list),(str(f),list(d))
 for r in rows:
  raw=Path(r['path'])
  ...
  add(p,{'sha256':r['sha256'],'byteLength':r.get('bytes',r.get('byteLength'))})
 add(f,{'sha256':actual,'byteLength':f.stat().st_size})
 freezes.append({'path':str(f),'sha256':actual,'declaredMembers':len(rows)})
for r in selection['closedProofSelections']:
 owner=A if r['repository']=='abiogenesis'else G
 if owner==repo:freeze(owner/r['freeze'],r['sha256'])
for r in selection['pendingReviewWipSelections']:freeze(A/r['carrier']/'freeze.json',r['freezeSha256'])
freeze(selection['writerReceipt'],'8972d3454c394961f1213669c313fb9e4ff36a07a21f189405e4937d34ce7723')
if repo==A:
 for p in selection['adjoiningClosedPosts']:add(A/'.ai-workspace/comments/codex'/p)
...
write('classification.json',{'baseCommit':activation['basisCommit'],'directPaths':sorted(selection['directExpected']),'priorTransportReceipts':receipts,'inheritedIndexedMembers':len(prior),'reusedSelectedMembers':list(reused.values()),'newArchiveMembers':len(rows),'newArchiveBytes':sum(r['byteLength']for r in rows),'selectedFreezes':freezes,'canonicalFreezeReferencesConservedThroughPostimages':sorted(set(canonical)),'pending':['Active WHOLE_PATH_SCOPE_REVIEW output; excluded until separately CLOSED','Original mutable GLC history/worksite and S02 resource trees; no snapshot selected'],'scope':'Accepted closed work plus explicitly pending-review conformance/retry WIP; not qualification/source-ready/release','priorPayloadReadOrReupload':False})
write('freeze-expectations.json',{p:r['expectedFreeze']for p,r in selected.items()})
write('exclusions.json',{'prefixesAndFiles':{},'active':['Active WHOLE_PATH_SCOPE_REVIEW output','original mutable GLC/S02 resources/worksite','unselected/unknown outputs'],'note':'No new blanket ignore of active or reproducible trees; only exact newly transported originals will be ignored.'})
