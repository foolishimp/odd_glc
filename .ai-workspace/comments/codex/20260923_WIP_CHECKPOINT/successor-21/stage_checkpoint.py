...
repo=Path(sys.argv[1]).resolve();prefix=Path('.ai-workspace/comments/codex/20260923_WIP_CHECKPOINT/successor-21');root=repo/prefix
...
for row in json.loads((root/'source-correspondence.json').read_text()):
 if row['repo']==repo.name:
  ...
  if row['headPreimageSha256'] is None:
   assert subprocess.run(['git','cat-file','-e','HEAD:'+row['path']],cwd=repo,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode!=0
  else:
