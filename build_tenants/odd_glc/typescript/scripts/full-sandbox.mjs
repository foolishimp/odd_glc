...
import { prepareFullSandbox, continueFullSandboxPreparation, executeFullSandbox, freshFullSandboxReadback } from "../test/full-sandbox-support.mjs";
...
if (mode === "prepare" && path !== undefined && extra !== undefined && configurationPath !== undefined) {
...
} else if (mode === "continue-prepare" && path !== undefined && extra !== undefined && configurationPath !== undefined) {
  console.log(JSON.stringify(await continueFullSandboxPreparation({ candidatePath: path, runRoot: extra, configurationPath }), null, 2));
  ...
  throw new TypeError("usage: full-sandbox.mjs prepare <frozen-candidate-basis.json> <empty-resource-root> <preparation-configuration.json> | continue-prepare <frozen-candidate-basis.json> <retained-resource-root> <same-preparation-configuration.json> | run <job/prepared.json> [prior-close-receipt.json] | read <job/prepared.json>");
}
