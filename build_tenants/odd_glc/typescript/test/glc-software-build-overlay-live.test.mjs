    ...
    stagePlan: [
      ...
      },
      {
        ...
        instructions: [
          ...
          "Per mutant: (1) confirm the FULL suite is green first (a red baseline invalidates every kill claim); (2) record the sha256 of THE FILE YOU WILL MUTATE as baselineDigest (format sha256:<64 lowercase hex>, e.g. via shasum -a 256) and name that file path inside mutantIdentity; (3) apply a minimal SEMANTIC mutation to PRODUCTION code (invert the guard, drop the rejection branch, weaken the invariant) — never mutate a test file and never a comment; (4) run the FULL sbt test suite from build_tenants/scala_spark with the same env binding as the execution stage; (5) set mutantCompiled true ONLY IF the mutant COMPILED and the suite actually ran (reports produced) — a compile error is mutantCompiled false and is NOT a kill (no test refuted it); (6) read the mutant run's XML reports and set failedTestIdentityRefs to the EXACT <testcase name> values that WENT RED under the mutant (the tests that actually failed — never the tests you targeted; if none failed the mutant SURVIVED, leave it empty); (7) RESTORE the original file exactly and record its sha256 as restoreDigest — must equal baselineDigest or the row is rejected; (8) record truthfully.",
          'Then write only mutation-outcomes.json: {"rows":[{"requirementId":"REQ-CDME-<CONCERN>","mutantIdentity":"<file path + short mutation description>","mutantCompiled":<true if it compiled and ran, false if compile error>,"failedTestIdentityRefs":["<exact testcase names that went RED under the mutant; empty = survived>"],"suiteExit":<the exit status you observed>,"baselineDigest":"sha256:<hex>","restoreDigest":"sha256:<hex>"}]}.',
          "CONSISTENCY (the kernel rejects contradictions): compile error => mutantCompiled false, failedTestIdentityRefs empty. Compiled with named failures => non-zero suiteExit. Compiled with no failures => suiteExit 0 (SURVIVED). A red suite with no named failing tests is not evidence and is rejected.",
          "A SURVIVED mutant (compiled, suite stayed green, no failed tests) is a counterexample: record it truthfully — it blocks closure and routes repair pressure to strengthen the tests. NEVER fabricate a kill; the kernel mints kill evidence only for the tests you list as actually failed.",
...
test("binding unit lane: depth payload lift — corroborated identities forward, unverified do not, payloads ride the artifact (T-032 Stage C)", async (t) => {
  ...
  await writeFile(path.join(caseRoot, "mutation-outcomes.json"), JSON.stringify({
    rows: [{ requirementId: "REQ-CDME-CORE", mutantIdentity: "cdme-core/package.scala: drop guard", mutantCompiled: true, failedTestIdentityRefs: ["CoreContractsSpec rejects malformed row"], suiteExit: 1, baselineDigest: "sha256:" + "ab".repeat(32), restoreDigest: "sha256:" + "ab".repeat(32) }]
  }), "utf8");
