...
| Parallel | Branch/fan-in work | Saga/frontier async execution | Parallel frontier lifecycle view | ABG frontier, branch lease, fan-in, aggregate fold | Branch/fan-in interpretation | Product-local branch scheduler | Sandbox and committed proof-input read exists | Fresh typed ABG 4.2 parallel startup proof. |
| Recursive | Program/project/task nesting | Local phase hierarchy and graph re-entry | Recursive span lifecycle view | ABG frame, span, zoom, foldback, re-entry | Span/foldback labels | Local hierarchy controller | T-027 active | Fresh typed recursive GLC proof. |
